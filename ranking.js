...
function obtenerNombreOficial(nombreOriginal) {
    ...
    const limpio = nombreOriginal.toLowerCase().trim();

    // 1. Buscar en el sistema real de equivalencias (Corrección de Nombres)
    ...
    if (equivalenciasGuardadas) {
        try {
            ...
            const buscadoExacto = lista.find(e => e.antiguo.toLowerCase() === limpio);
            const buscado = buscadoExacto || lista.find(e => e.antiguo.toLowerCase().includes(limpio) || limpio.includes(e.antiguo.toLowerCase()));
            if (buscado) {
    ...
    // 2. Fallback: mapa de correcciones antiguo (compatibilidad, por si se usa en el futuro)
